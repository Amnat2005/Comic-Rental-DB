...
<head>
    <meta charset="UTF-8">
    <title>รายการประเภทหนังสือ (Genre)</title>
    <style>
...
<body>
    ...
    <div class="toolbar">
        <div class="title">รายการประเภทหนังสือ (Genre)</div>
        <div class="actions">
            <a href="../../index.php" class="back-button">🏠 กลับหน้าหลัก</a>
            <a href="add.php" class="btn">➕ เพิ่มประเภทหนังสือใหม่</a>
        </div>
    </div>

    <!-- Search + Order -->
    <div class="table-control">
        <input type="text" id="searchInput" placeholder="ค้นหาประเภทหนังสือ..." onkeyup="filterTable()">
        <div class="order-container">
            <label for="orderSelect">เรียงตาม</label>
            <select id="orderSelect" onchange="orderTable()">
                <option value="genre_id">ID</option>
                <option value="genre_name">ชื่อประเภท</option>
            </select>
        </div>
    </div>
        ...
        <table id="genreTable">
            ...
            <tbody>
                <?php while ($row = $result->fetch_assoc()) { ?>
                    <tr>
                        <td data-label="ID"><?= htmlspecialchars($row['genre_id']) ?></td>
                        <td data-label="ประเภท"><?= htmlspecialchars($row['genre_name']) ?></td>
                        <td class="td-actions" data-label="Action">
                            <a href="edit.php?id=<?= $row['genre_id'] ?>">Edit</a>
                            <a href="delete.php?id=<?= $row['genre_id'] ?>" onclick="return confirm('Delete this genre?')">Delete</a>
                        </td>
                    </tr>
                <?php } ?>
            </tbody>
    ...
    <script>
        ...
        function orderTable() {
            const table = document.getElementById("genreTable").tBodies[0];
            const rows = Array.from(table.rows);
            const key = document.getElementById("orderSelect").value;
            rows.sort((a,b)=>{
                let aText="",bText="";
                if(key==="genre_id"){ aText=parseInt(a.cells[0].textContent); bText=parseInt(b.cells[0].textContent);}
                else if(key==="genre_name"){ aText=a.cells[1].textContent.toLowerCase(); bText=b.cells[1].textContent.toLowerCase();}
