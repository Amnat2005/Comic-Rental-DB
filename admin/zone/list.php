...
<div class="toolbar">
    <div class="title">รายการโซน (Zone)</div>
    <div class="actions">
        <a href="../../index.php" class="back-button">🏠 กลับหน้าหลัก</a>
        <a href="add.php" class="btn">➕ เพิ่มโซน</a>
    </div>
</div>

<div class="table-control">
    <input type="text" id="searchInput" placeholder="ค้นหาโซน..." onkeyup="filterTable()">
    <div class="order-container">
        <label for="orderSelect">เรียงตาม</label>
        <select id="orderSelect" onchange="orderTable()">
            <option value="zone_id">ID</option>
            <option value="zone_name">ชื่อโซน</option>
        </select>
    </div>
</div>
...
<table id="zoneTable">
    ...
    <?php while ($row = $result->fetch_assoc()) { ?>
        <tr>
            <td data-label="ID"><?= htmlspecialchars($row['zone_id']) ?></td>
            <td data-label="ชื่อโซน"><?= htmlspecialchars($row['zone_name']) ?></td>
            <td class="td-actions" data-label="Action">
                <a href="edit.php?id=<?= $row['zone_id'] ?>">Edit</a>
                <a href="delete.php?id=<?= $row['zone_id'] ?>" onclick="return confirm('ต้องการลบโซนนี้หรือไม่?')">Delete</a>
            </td>
        </tr>
    <?php } ?>
    </tbody>
...
function orderTable() {
    const table = document.getElementById("zoneTable").tBodies[0];
    const rows = Array.from(table.rows);
    const key = document.getElementById("orderSelect").value;
    const colIndexMap = { "zone_id":0, "zone_name":1 };
