...
$sql = "SELECT e.*, j.position_name, m.first_name AS manager_first, m.last_name AS manager_last
        FROM Employees e
...
<head>
    <meta charset="UTF-8">
    <title>รายการพนักงาน (Employees)</title>
    <style>
...
<body>
    ...
    <div class="toolbar">
        <div class="title">รายการพนักงาน (Employees)</div>
        <div class="actions">
            <a href="../../index.php" class="back-button">🏠 กลับหน้าหลัก</a>
            <a href="add.php" class="btn">➕ เพิ่มพนักงานใหม่</a>
        </div>
    </div>

    <!-- Search + Order -->
    <div class="table-control">
        <input type="text" id="searchInput" placeholder="ค้นหาพนักงาน..." onkeyup="filterTable()">
        ...
        <table id="employeeTable">
            ...
            <tbody>
                <?php while ($row = $result->fetch_assoc()) { ?>
                    <tr>
                        <td data-label="ID"><?= $row['employee_id'] ?></td>
                        <td data-label="ชื่อ-สกุล"><?= htmlspecialchars($row['first_name'] . " " . $row['last_name']) ?></td>
                        <td data-label="ตำแหน่ง"><?= htmlspecialchars($row['position_name']) ?></td>
                        <td data-label="วันเริ่มงาน"><?= htmlspecialchars($row['hire_date']) ?></td>
                        <td data-label="สถานะ"><?= htmlspecialchars($row['employment_status']) ?></td>
                        <td data-label="ผู้จัดการ">
                            <?= $row['manager_first'] ? htmlspecialchars($row['manager_first'] . " " . $row['manager_last']) : "-" ?>
                        </td>
                        <td class="td-actions" data-label="Action">
                            <a href="edit.php?id=<?= $row['employee_id'] ?>">Edit</a>
                            <a href="delete.php?id=<?= $row['employee_id'] ?>" onclick="return confirm('Delete this employee?')">Delete</a>
                        </td>
                    </tr>
                <?php } ?>
            </tbody>
    ...
    <script>
        ...
        function orderTable() {
            const table = document.getElementById("employeeTable").tBodies[0];
            const rows = Array.from(table.rows);
            const key = document.getElementById("orderSelect").value;
            rows.sort((a,b)=>{
                let aText="",bText="";
                if(key==="employee_id"){ aText=parseInt(a.cells[0].textContent); bText=parseInt(b.cells[0].textContent);}
                else if(key==="first_name"){ aText=a.cells[1].textContent.toLowerCase(); bText=b.cells[1].textContent.toLowerCase();}
                else if(key==="position_name"){ aText=a.cells[2].textContent.toLowerCase(); bText=b.cells[2].textContent.toLowerCase();}
